/*
Exercise 8-8. Write a routine bfree(p,n) that will free any arbitrary block p
...
int main(void)
{
    ...
    size_t newhs = 10;
    Header newh[newhs];
    if (bfree(newh, newhs * sizeof(Header)))
        visualize("bfree");
    ...
    if (memres == 0)
        ...
        printf("    (!) 'bfree' added memory : %ld chunks (%ld bytes).\n",
                    (freemem - totmem), (freemem - totmem) * sizeof(Header));
...
{
    if (n < sizeof(Header))                 /* too small */
        return 0;                           /* error */

    Header *tmp = (Header *) p;
    tmp->s.size = n / sizeof(Header);       /* truncate */

    void *t = (void *)(tmp + 1);
    mfree(t);                               /* add to the list */
